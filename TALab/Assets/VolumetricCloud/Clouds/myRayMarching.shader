Shader "URPCustom/Volume/myRayMarching" {
    ...
    SubShader {
        ...
        float SampleCloudDensity(float3 currentPos) {
            ...
            float4 weatherData = SAMPLE_TEXTURE2D_LOD(_WeatherTex, sampler_WeatherTex, weatherTexUV * 0.000001 + _WeatherTex_ST.zw + wind.xz * 0.01, 0);
            ...
            float stratusDensity = GetCloudTypeDensity(heightFraction, _stratusInfo.x, _stratusInfo.y, _stratusInfo.w);
            float cumulusDensity = GetCloudTypeDensity(heightFraction, _cumulusInfo.x, _cumulusInfo.y, _cumulusInfo.w);
        ...
        float3 lightmarchEarth(float3 position, float dstTravelled) {
            ...
            float dstInsideSphere = RayCloudLayerDst(EarthCenter, EarthRadius, _CloudHeightRange.x, _CloudHeightRange.y, position, lightDir, false).y;
        ...
        float3 Scattering(float tau, float cos, float3 Li, float stepsize, float density) {
            ...
            for (int n = 0; n < _octaves; n++) {
                ...
                if (n == 0 && _embientON != 0) {
                    ...
                    float3 luminance0 = _sigma_s * _embientColor * b * Beer(tau, a * _sigma_t) * pow(1 - density, 0.5) * density * _AmbinetScale0;
                    float3 luminance1 = _sigma_s * _embientColor * b * Beer(tau, a * _sigma_t) * pow(1 - density, 0.5) * 0.003 * _AmbinetScale1;
                ...
                //directLight
                luminance += _DirectLightMultiplier * (_sigma_s * density) * b * Li * phase(cos, g * c) * Beer(tau, a * _sigma_t);
                a *= Attenuation;
                b *= Contribution;
        ...
        float4 cloudRayMarchingEarth(float3 startPoint, float3 direction, float dstToCloud, float inCloudMarchLimit, float2 uv) {
            ...
            float blueNoise = SAMPLE_TEXTURE2D_LOD(_BlueNoiseTex, sampler_BlueNoiseTex, uv * _BlueNoiseTex_ST.xy, 0).r;
            if (_DitheringON == 1)
        ...
        Pass {
            ...
            half4 frag(v2f i) : SV_TARGET {
                ...
                float2 rayHitCloudInfo = RayCloudLayerDst(EarthCenter, EarthRadius, _CloudHeightRange.x, _CloudHeightRange.y, camPos, viewDir);
